...
DB_PATH = str(current_dir / os.environ.get("DATABASE_PATH", "week2/data/jobs_d3_eval.db"))
...
def chat_endpoint(payload: ChatRequest):
    ...
    if not message and not pdf_text:
        return JSONResponse(status_code=400, content={"response": "Please provide a prompt or a PDF."})
    intent_prompt = (
    ...
    if intent == "yes":
        if not pdf_text:
            return JSONResponse(content={"response": "Please upload a PDF resume to run a Skill Gap Analysis."})
